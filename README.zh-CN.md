...
打开 `cineops-demo/cineops-demo.html`，即可对比同一份单镜头交接在修复前后的结果。演示会显示 CineOps 如何发现断开的引用、非法时长、缺失的连续性状态和缺失的生成前审查。报告是完全本地的单文件 HTML，不上传制作资料，也不加载外部脚本、字体或统计服务。
...
`cineops report` 会把相同的确定性检查结果生成带轻量动画的可视化报告，方便团队快速查看错误、警告、镜头数量、审查覆盖和发布门禁状态。
